...
//! `FrameDecoder::decode_all_magicless` reproduces the input.
...
fn round_trip(plain: &[u8]) {
    ...
    let written = decoder
        .decode_all_magicless(&magicless, &mut out)
        .expect("decode_all_magicless");
...
fn magicless_decoder_rejects_short_input() {
    ...
    let mut decoder = FrameDecoder::new();
    let mut out = [0u8; 16];
    assert!(decoder.decode_all_magicless(&[0x00], &mut out).is_err());
}
